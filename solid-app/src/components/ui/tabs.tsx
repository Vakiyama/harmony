...
export const TabsList = <T extends ValidComponent = "div">(
  ...
  return (
    <TabsPrimitive.List
      class={cn(
        "flex w-full rounded-lg bg-muted p-1 text-muted-foreground data-[orientation=vertical]:flex-col data-[orientation=horizontal]:items-center data-[orientation=vertical]:items-stretch",
        local.class
      )}
      {...rest}
    />
...
export const TabsTrigger = <T extends ValidComponent = "button">(
  ...
  return (
    <TabsPrimitive.Trigger
      class={cn(
        "peer relative inline-flex h-7 w-full text-h4 font-medium items-center justify-center whitespace-nowrap rounded-md px-3 py-1 font-medium outline-none transition-colors disabled:pointer-events-none disabled:opacity-50 data-[selected]:text-foreground",
        local.class
      )}
      {...rest}
    />
